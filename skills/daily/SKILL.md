...
/daily report is done, finished it yesterday
/daily idea: we should try the alternative approach
/daily skip the meeting prep, moved to next week
```
...
Read the full note so you understand what was briefed (narrative, deadlines).
...
- "report is done, finished yesterday"
- "skip the deadline question, I already handled it"
- "idea: what if we restructure the intro section"
- "meeting with Sarah at 2pm, she wants the update by Friday"

If the user invoked `/daily` with no arguments, read the briefing back as a short summary and ask: "What's the update?"
...
1. Find the matching `#due/YYYY-MM-DD` line in the SOURCE file (the file where the task lives, not the briefing note)
...
- If the idea clearly belongs to a specific file or project, mention the relevant note in the log so links exist
...
- Log the answer
- If the answer implies an action, log that as a commitment

### Step 4: Suggest next steps

After processing all items, look at what remains undone from the briefing deadlines. Suggest the natural next action based on:
...
**Important:** After writing to the journal note, copy the entire updated file to `Daily Briefing.md` in the vault root. Both files must stay in sync.
...
```
Ticked off 2 items in [[ProjectPlan]]. Logged 1 idea.
Next up: review draft (due tomorrow).
```
...
1. **Creating a task:** Write `- [ ] Do the thing #due/2026-06-15` in any vault file. That's it. The briefing will find it.
2. **Briefing picks it up:** The morning briefing scans every file for `#due/` tags and includes them by urgency.
3. **User says it's done:** Via `/daily`, voice, or typing: "the thing is done."
4. **Claude updates the source:** In the original file, `#due/2026-06-15` becomes `#done/2026-06-15` and the checkbox gets ticked.
5. **Tomorrow's briefing ignores it:** Lines with `#done/` are filtered out.

No special file format required. No thread files. No kanban board. Just write `#due/YYYY-MM-DD` anywhere and the system picks it up.
...
- **Handle messy voice input gracefully.** "finsihed the reprot ysterday" means the report was finished yesterday.
